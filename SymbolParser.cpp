...
void parseSymbolSet(std::bitset<256> &column, std::string symbol_set) {
	...
	if ((symbol_set[0] == '{') && (symbol_set[symbol_set.size() - 1] == '}')) {
	...
	while (index < symbol_set.size()) {
		...
		case 'x':
			if (escaped) {
				...
				hex[2] = '\0';
				unsigned char number = (unsigned char) std::strtoul(hex, NULL,
						16);
	...
	if (bracket_sem != 0 || brace_sem != 0) {
		std::cout << "MALFORMED BRACKETS OR BRACES: " << symbol_set
				<< std::endl;
